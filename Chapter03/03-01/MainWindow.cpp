...
void MainWindow::open()
{
    if ( okToContinue() )
    {
        QString fileName = QFileDialog::getOpenFileName( this, tr(""), ".", tr("") );
        if ( !fileName.isEmpty() )
        {
            loadFile( fileName );
        }
...
{
    QString fileName = QFileDialog::getSaveFileName( this, tr(""), ".", tr("") );
    if ( fileName.isEmpty() )
    {
        return false;
    }
...
void MainWindow::find()
{
    if ( !findDialog )
    {
        ...
        QObject::connect( findDialog, SIGNAL( findNext( const QString&, Qt::CaseSensitivity ) ),
                spreadsheet, SLOT( findNext( const QString&, Qt::CaseSensitivity ) ) );

        QObject::connect( findDialog, SIGNAL( findPrevious( const QString&, Qt::CaseSensitivity ) ),
...
void MainWindow::goToCell()
{
    ...
    if ( dialog.exec() )
    {
        Ui::SortDialog* ui = dialog.getUI();


        SpreadSheetCompare compare;
        compare.keys[0] = ui->primaryColumnCombo->currentIndex();
        compare.keys[1] = ui->secondaryColumnCombo->currentIndex() - 1;
        compare.keys[2] = ui->tertiaryColumnCombo->currentIndex() - 1;
        compare.ascending[0] = ( ui->primaryOrderCombo->currentIndex() == 0 );
        compare.ascending[1] = ( ui->secondaryColumnCombo->currentIndex() == 0 );
        compare.ascending[2] = ( ui->tertiaryColumnCombo->currentIndex() == 0 );
